...
int main()
{
	...
	// 세번째, 네번째 인자 : low 임계값, high 임계값
	// low 임계값보다 작고, high 임계값 보다 클 때 edge로 인식함.
	// high 임계값이 클수록(적당히) 직선이 깔끔하게 검출되는 것 같다.

	std::vector<cv::Vec2f> lines;
	
	cv::HoughLines(img_canny, lines, 1, CV_PI / 180, 150);
	// 제일 마지막 인자(150) : 교점의 갯수
	// 작을수록 더 많은 선이 검출
	cv::Mat img_hough;
	...
	while (1) {
		...
		for (size_t i = 0; i < lines.size(); i++)
		{
			...
			double alp = (double)(pointLine_b[i].y - pointLine_a[i].y) / (double)(pointLine_b[i].x - pointLine_a[i].x); // 기울기, 강제캐스팅해줘야함
